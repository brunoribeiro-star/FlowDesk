...
type Role = "freelancer" | "contratante";
...
export default function SignupPage() {
  ...
  async function handleSignup(e: React.FormEvent<HTMLFormElement>) {
    ...
    const { data, error: signUpError } = await supabase.auth.signUp({
      email,
      password,
      options: { data: { nome, name: nome, role } },
    });
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            <h1 className="text-lg font-semibold" style={{ color: "var(--gray-100)" }}>
              {step === 1 ? "Criar conta" : "Tipo de conta"}
            </h1>
          ...
          <div className="flex items-center gap-1.5">
            <div
              ...
              style={{ background: step >= 1 ? "var(--primary-500)" : "var(--primary-700)" }}
            />
            <div
              className="w-6 h-1.5 rounded-full transition-colors"
              style={{ background: step >= 2 ? "var(--primary-500)" : "var(--primary-700)" }}
            />
          </div>
        </div>
          ...
          <form onSubmit={handleStep1} className="flex flex-col gap-4">
            ...
            <button
              ...
              style={{
                ...
                opacity: emailError || checkingEmail ? 0.5 : 1,
              }}
            >
              Continuar
            </button>
        ...
        {step === 2 && (
          <form onSubmit={handleSignup} className="flex flex-col gap-5">
            <p className="text-sm" style={{ color: "var(--gray-300)" }}>
              Como você vai usar o FlowDesk?
            </p>

            <div className="flex flex-col gap-3">
              {(
                [
                  { value: "freelancer", label: "Freelancer", desc: "Gerencio projetos e atendo clientes" },
                  { value: "contratante", label: "Contratante", desc: "Contrato freelancers para meus projetos" },
                ...
                <button
                  ...
                  className="flex items-start gap-3 p-4 rounded-xl text-left transition-colors w-full"
                  style={{
                    background:
                      role === option.value
                ...
                >
                  ...
                  <div>
                    <p
                      ...
                      style={{ color: role === option.value ? "var(--primary-400)" : "var(--gray-100)" }}
                    >
                      {option.label}
                    </p>
                    <p className="text-xs mt-0.5" style={{ color: "var(--gray-400)" }}>
            ...
            <div className="flex gap-2 mt-1">
              <button
                ...
                className="h-11 px-4 rounded-xl text-sm flex items-center gap-1.5 transition-colors"
                style={{
                  background: "var(--primary-800)",
                  border: "1px solid var(--primary-700)",
                  color: "var(--gray-300)",
