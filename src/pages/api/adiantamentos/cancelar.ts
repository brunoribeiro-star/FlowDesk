...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { data: adiantamento, error: adErr } = await supabaseAdmin
    .from("adiantamentos")
    .select("id, projeto_id, user_id, status, valor")
  ...
  if (adiantamento.status !== "solicitado") return res.status(400).json({ error: "Apenas solicitações pendentes podem ser canceladas." });
